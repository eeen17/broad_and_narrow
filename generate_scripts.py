...
for checkpoint in eval_checkpoints:
    for base in bases:
        for n_digits in eval_n_digits:
            for model in models:
                commands = f"python eval_pipe.py --base {base} --model_name outputs/{model}/checkpoint-{checkpoint} --size 300 --n_digits {n_digits}"
                with open(command_file, "a") as f:
                    f.write(command + "\n")
# and baseline evals
for base in bases:
    for n_digits in eval_n_digits:
        commands = f"python eval_pipe.py --base {base} --model_name unsloth/Phi-4 --size 300 --n_digits {n_digits}"
        with open(command_file, "a") as f:
            f.write(command + "\n")
